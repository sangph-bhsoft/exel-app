...
function App() {
  ...
  const handleDownload = () => {
    ...
    ws.columns = [
      ...
      { header: 'Mã định danh Bộ GD&ĐT', key: 'Mã định danh Bộ GD&ĐT', width: 15 },
    ...
    // if (studentSources.length !== studentDestinations.length) {
    //   let stName = "";
    //   const listNames = studentSources.map((item) =>
    //     item["Họ tên"].normalize("NFC")
    //   );
    //   studentDestinations.forEach((st) => {
    //     if (!listNames.includes(st["Họ và tên"].normalize("NFC"))) {
    //       stName += st["Họ và tên"] + ", ";
    //       console.log(listNames, st["Họ và tên"]);
    //     }
    //   });
    //   stName = stName.trim();
    //   window.alert(
    //     `Số lượng học sinh 2 file đang khác nhau, Vui lòng kiểm tra lại học sinh ${stName} đang không có trong file mẫu`
    //   );
    //   return;
    // }
    ...
    const studentRenders = studentSources.map((st) => {
      ...
      if (student) {
        return {
          ...
          "Công nghệ": student["CÔNG_NGHỆ"] ? student["CÔNG_NGHỆ"] : student["CÔNG NGHỆ"],
        ...
        <div className="space-y-2 w-full">
          ...
          <div className="flex items-center rounded-lg overflow-hidden">
            <input
              ...
              name="source"
            />
            <input className="flex-1 px-2 py-2 text-purple-600 italic outline-none focus:outline-none" readOnly value={file.source} />
            <button
        ...
        <div className="space-y-2 w-full">
          ...
          <div className="flex items-center rounded-lg overflow-hidden">
            <input
              ...
              name="destination"
            />
            <input className="flex-1 px-2 py-2 text-purple-600 italic outline-none focus:outline-none" readOnly value={file.destination}/>
            <button
